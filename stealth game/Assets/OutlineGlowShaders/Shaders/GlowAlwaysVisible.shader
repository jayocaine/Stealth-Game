Shader "VaxKun/GlowAlwaysVisible"{
	Properties {
	...
	OuterGlow vertPassGlobal(InnerGlow v, fixed Occlusion) {
		...
		resizeMatrix[0][0] = 1.0 + (_Outline*Occlusion*_OuterScale.r); resizeMatrix[1][1] = 1.0 + (_Outline*Occlusion*_OuterScale.g); resizeMatrix[2][2] = 1.0 + (_Outline*Occlusion*_OuterScale.b);
	...
	SubShader {				
		...
		Pass {
			Name "OUTLINE01" Tags { "LightMode" = "Always" "Queue" = "Transparent" }
		...
		Pass {
			Name "OUTLINE02" Tags { "LightMode" = "Always" "Queue" = "Transparent" }
			Cull Off ZWrite Off ZTest Always Blend SrcAlpha OneMinusSrcAlpha 
			CGPROGRAM			
			#pragma vertex vertPass
			#pragma fragment fragPass
		...
		Pass {
			Name "OUTLINE03" Tags { "LightMode" = "Always" "Queue" = "Transparent" }
			Cull Off ZWrite Off ZTest Always Blend SrcAlpha OneMinusSrcAlpha 
			CGPROGRAM			
			#pragma vertex vertPass
			#pragma fragment fragPass
		...
		Pass {
			Name "OUTLINE04" Tags { "LightMode" = "Always" "Queue" = "Transparent" }
			Cull Off ZWrite Off ZTest Always Blend SrcAlpha OneMinusSrcAlpha 
			CGPROGRAM			
			#pragma vertex vertPass
			#pragma fragment fragPass
